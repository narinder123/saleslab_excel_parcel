...
export const Helpers = new (class helperFunction {
  ...
  getPremiumResidencyArr(residency: string): string[] {
    if(residency.length == 2 ) return [residency];
    if(!customResidencies[residency]){
      throw `"${residency}" residency doesn't exist! please use these defined residencies: ${customResidencies}`;
    }
    return customResidencies[residency];
  }
